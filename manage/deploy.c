...
int main(int argc, char **argv)
{
    ...
    if (argc > 1)
    { // Second param - existing master key
    ...
    if (argc > 2)
    { // Third param - set master key
        uint8_t read_picc_master_key[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
        parse_key(read_picc_master_key, argv[3]);
        memcpy(new_picc_master_key, read_picc_master_key, sizeof(new_picc_master_key));
    }
    else
    {
    ...
    printf("Using as new master key: ");
    print_buffer(stdout, new_picc_master_key, 16);
    printf("\n");

    if (argc > 3)
    { // Fourth param - set AMK
        uint8_t read_app_master_key[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
        parse_key(read_app_master_key, argv[4]);
        memcpy(new_app_master_key, read_app_master_key, sizeof(new_app_master_key));
    }
    else
    {
    ...
    printf("Using as new AMK key: ");
    print_buffer(stdout, new_app_master_key, 16);
    printf("\n");

    if (argc > 4)
    { // Fourth param - set AppKey 0xD
        uint8_t read_app_key[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
        parse_key(read_app_key, argv[5]);
        memcpy(new_app_key, read_app_key, sizeof(new_app_key));
    }
    else
    {
    ...
    printf("Using as new application key: ");
    print_buffer(stdout, new_app_key, 16);
    printf("\n");
    ...
    json_key("ca0523_door_key", new_app_key);
    printf(",\n");
    json_key("ca0523_master_key", new_app_master_key);
    printf(",\n");
    ...
    json_key("picc_key", new_picc_master_key);
    printf(",\n");
    printf("   \"uid\" : \"%s\"\n", tag_uid);
    printf("}\n");
